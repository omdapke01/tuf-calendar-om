...
export const weekdayLongLabels = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
...
export function formatDisplayDate(key) {
  ...
  return `${weekdayLabels[(date.getDay() + 6) % 7]}, ${monthShortLabels[date.getMonth()]} ${date.getDate()}, ${date.getFullYear()}`;
}

export function formatAriaDate(key) {
  if (!key) return "";
  const date = fromDateKey(key);
  return `${weekdayLongLabels[date.getDay()]}, ${monthLabels[date.getMonth()]} ${date.getDate()}, ${date.getFullYear()}`;
}
